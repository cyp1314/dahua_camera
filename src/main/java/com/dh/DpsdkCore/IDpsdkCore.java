...
//	static public native  int	DpsdkGetLastError();
     ...
     * @return 函数返回错误类型，参考dpsdk_retval_e
     * @remark 1、pDepInfo、pDeviceInfo需要在外面创建好
     ...
     * @param IN  nTimeout		超时时长，单位毫秒
     * @return 函数返回错误类型，参考dpsdk_retval_e
     * @remark 1、nRecordCount最大5000个记录
     */
     ...
     * @return 函数返回错误类型，参考dpsdk_retval_e
     * @param     IN    nPDLLHandle		SDK句柄
     * @remark lSeekBegin在文件模式下的计算方式可以是:(文件大小值)/100*(定位处相对文件的百分比)
     */
     ...
     * @return 函数返回错误类型，参考dpsdk_retval_e
     * @remark 1、alarmInChnlInfo需要在外面创建好
     ...
     * @return 函数返回错误类型，参考dpsdk_retval_e
     * @remark 1、defenceStatus需要在外面创建好，根据通道个数new
     */
     ...
     * @remark 1、布控时需要明白不同的报警类型对应不同的参数
     */
     ...
     * @return 函数返回错误类型，参考{@link dpsdk_retval_e}
     * @sample Return_Value_Info_t nCount = new Return_Value_Info_t();
     ...
     * @param        IN        nTimeout			超时时间
     * @sample DPSDK_SendScsMsg(m_nPDLLHandle, " test " .getBytes (), "test".getBytes(), dpsdk_constant_value.DPSDK_CORE_DEFAULT_TIMEOUT);
     */
